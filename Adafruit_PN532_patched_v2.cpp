...
#define PN532_PACKBUFFSIZ 384                ///< Packet buffer size in bytes
...
bool Adafruit_PN532::inDataExchange(uint8_t *send, uint8_t sendLength,
  ...
  // Read a fixed maximum into the packet buffer, then parse actual frame length.
  readdata(pn532_packetbuffer, (uint16_t)sizeof(pn532_packetbuffer));

  // Basic frame sanity: preamble + start codes
  ...
  // Parse normal vs extended length.
  uint16_t frameLen = 0;     // length of TFI+PD bytes
  uint16_t dataOff  = 0;     // offset to TFI
  if (pn532_packetbuffer[3] == 0xFF && pn532_packetbuffer[4] == 0xFF) {
    // Extended frame: 00 00 FF FF FF LENhi LENlo LCS2 [TFI+PD..] DCS 00
    uint8_t lenHi = pn532_packetbuffer[5];
    uint8_t lenLo = pn532_packetbuffer[6];
    uint8_t lcs2  = pn532_packetbuffer[7];
    if ((uint8_t)(lenHi + lenLo + lcs2) != 0x00) {
#ifdef PN532DEBUG
      PN532DEBUGPRINT.println(F("Extended length check invalid"));
#endif
      return false;
    }
    frameLen = ((uint16_t)lenHi << 8) | lenLo;
    dataOff  = 8;
  } else {
    // Normal frame: 00 00 FF LEN LCS [TFI+PD..] DCS 00
    uint8_t len1 = pn532_packetbuffer[3];
    uint8_t lcs1 = pn532_packetbuffer[4];
    if ((uint8_t)(len1 + lcs1) != 0x00) {
#ifdef PN532DEBUG
  ...
  // Ensure indices fit into our packet buffer.
  uint32_t dcsIndex  = (uint32_t)dataOff + (uint32_t)frameLen;
  uint32_t postIndex = dcsIndex + 1;
  if (postIndex >= (uint32_t)sizeof(pn532_packetbuffer)) {
#ifdef PN532DEBUG
    PN532DEBUGPRINT.println(F("Frame too large for packet buffer"));
#endif
    return false;
  }

  // Optional DCS check (recommended when pushing large frames).
  uint8_t sum = 0;
  for (uint16_t i = 0; i < frameLen; i++) {
    sum += pn532_packetbuffer[dataOff + i];
  }
  sum += pn532_packetbuffer[dcsIndex];
  if (sum != 0x00) {
#ifdef PN532DEBUG
    PN532DEBUGPRINT.println(F("Data checksum (DCS) invalid"));
#endif
    return false;
  }

  // Validate response header and extract APDU payload.
  if (pn532_packetbuffer[dataOff] == PN532_PN532TOHOST &&
      pn532_packetbuffer[dataOff + 1] == PN532_RESPONSE_INDATAEXCHANGE) {

    uint8_t status = pn532_packetbuffer[dataOff + 2];
    if ((status & 0x3F) != 0x00) {
#ifdef PN532DEBUG
    ...
    uint16_t apduLen = frameLen - 3; // remove TFI + response code + status

    resp_len_t maxOut = *responseLength;
    resp_len_t ncopy =
        (apduLen > (uint16_t)maxOut) ? maxOut : (resp_len_t)apduLen;

    for (resp_len_t i = 0; i < ncopy; ++i) {
      response[i] = pn532_packetbuffer[dataOff + 3 + i];
    }
    *responseLength = ncopy;
    return true;
  }

#ifdef PN532DEBUG
  PN532DEBUGPRINT.print(F("Don't know how to handle this command: "));
  PN532DEBUGPRINT.println(pn532_packetbuffer[dataOff + 1], HEX);
#endif
  return false;
}
...
bool Adafruit_PN532::inDataExchange(uint8_t *send, uint8_t sendLength,
  ...
  resp_len_t rlen = *responseLength;
  bool ok = inDataExchange(send, sendLength, response, &rlen);
  *responseLength = (rlen > 0xFF) ? 0xFF : (uint8_t)rlen;
  return ok;
}
#endif


// === ISO14443-4 Parameter Selection (PSL/PPS) ==============================
bool Adafruit_PN532::inPSL(uint8_t tg, uint8_t brit, uint8_t brti) {
  // Yêu cầu: Đã "inlist" được target (Tg) trước đó
  pn532_packetbuffer[0] = PN532_COMMAND_INPSL; // 0x4E
  pn532_packetbuffer[1] = tg;                  // Thường = 1
  pn532_packetbuffer[2] = brit;                // Initiator -> Target (DSI)
  pn532_packetbuffer[3] = brti;                // Target -> Initiator (DRI)
  ...
  // Đọc phản hồi: D5 4F <Status>
  // Theo convention của lib: [5]=0xD5, [6]=0x4F, [7]=Status
  readdata(pn532_packetbuffer, 8);

  if ((pn532_packetbuffer[5] != PN532_PN532TOHOST) ||
      (pn532_packetbuffer[6] != PN532_RESPONSE_INPSL)) {
    return false;
  }
...
void Adafruit_PN532::readdata(uint8_t *buff, uint16_t n) {
  // Defensive: never read more than our packet buffer can reasonably hold.
  if (n > PN532_PACKBUFFSIZ) {
    n = PN532_PACKBUFFSIZ;
  }
  ...
  if (spi_dev) {
    ...
    // I2C read (+1 for leading RDY byte)
    static uint8_t rbuff[PN532_PACKBUFFSIZ + 1];

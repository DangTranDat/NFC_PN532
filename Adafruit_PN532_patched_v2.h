...
#ifndef PN532_RESP_LEN_16
#define PN532_RESP_LEN_16 1
#endif

// If the sketch defined resp_len_t as a macro (Arduino auto-prototype workaround),
// undefine it here so we can use a real typedef without breaking the library.
#ifdef resp_len_t
#undef resp_len_t
#endif

#if PN532_RESP_LEN_16
typedef uint16_t resp_len_t;
#else
typedef uint8_t  resp_len_t;
#endif
...
#define PN532_RESPONSE_INPSL          (0x4F)
...
#if PN532_RESP_LEN_16
  // Backward compatible overload (8-bit length) when resp_len_t is 16-bit.
  bool inDataExchange(uint8_t *send, uint8_t sendLength, uint8_t *response,
                      uint8_t *responseLength);
#endif

  bool inListPassiveTarget();
    // --- ISO14443-4 Parameter Selection (PPS/PSL) ---
  // Trả về true nếu Status == 0x00
  bool inPSL(uint8_t tg, uint8_t brit, uint8_t brti);

  // Tiện ích: đặt cùng tốc độ hai chiều
  inline bool inPSL(uint8_t tg, uint8_t br) { return inPSL(tg, br, br); }
